...
import {
  ...
  TrackPoint,
  CommentEntity,
} from "../domain/domainClasses.js";
import {
  retrieveJson,
  retrieveAllRoutes,
  storeJSONToPOD,
} from "./PODExtractor.js";
...
export const loadMapInfo = async (jsonUrl) => {
  ...
  if (!foundErrorOnParse) {
    ...
    for (var key in routeJson) {
      ...
      if (key === "description") routeDescription = value;
      ...
      if (key === "comments") {
        ...
        await loadCommentsFromRouteCommentsProperty(commentsFile).then(
          (resultCommentList) => {
...
const parseJsonFromTtl = (ttlSource, fileName) => {
  ...
  if (jsonFromLib["@graph"]) {
    ...
    jsonRoutePoints = jsonFromLib["@graph"][0]["viade:point"].map(function(
      each
    ) {
      return {
        latitude: parseFloat(each["schema:latitude"]["@value"]),
        longitude: parseFloat(each["schema:longitude"]["@value"]),
      };
    });
  ...
  storeJSONToPOD(routeJsonLD, function(res) {}, fileName);
...
export const loadAllRoutes = async (personWebId) => {
  ...
  if (filesObj.files) {
    var onlyJson = filesObj.files
      .filter(function(urlMap) {
...
export const loadFriendRoutes = async (webId, filename) => {
  var routeUri =
    webId.substring(0, webId.length - 16) +
    "/viade/shared/" +
    filename +
    ".jsonld";
...
export async function loadCommentsFromRouteCommentsProperty(routeCommentsFile) {
  ...
  for (var i = 0; i < comentarios.length; i++) {
    commentList.push(
      new CommentEntity(
        comentarios[i].text,
        comentarios[i].dateCreated,
        comentarios[i].author
      )
    );
  }

...
defineFeature(feature, (test) => {
  ...
  test("Alice wants to add an existing user as a friend", ({
    given,
    when,
    then,
  }) => {
    given(
      "Alice does not have Bob as a friend and Bob's webID exists",
      async () => {
        await delay(10000);
    ...
    when(
      "Alice writes Bob's webID and clicks the button Add Friend",
      async () => {
        await delay(10000);
    ...
    then(
      "Bob is made Alice's friend and the Manage Friends view is reloaded",
      async () => {
        await delay(50000);
  ...
  test("Alice wants to add a non-existing user as a friend", ({
    given,
    when,
    then,
  }) => {
    ...
    when(
      "Alice writes NonExistingUser's webID and clicks the button Add Friend",
      async () => {
        await delay(10000);
        await page.waitForSelector('input[id="webId"]');
        await expect(page).toFill(
    ...
    then(
      "an alert indicating the error is shown and the user is not added as a friend",
      async () => {
        await delay(10000);
        await expect(page).not.toMatchElement(
          'button[id="buttonFriendhttps://ldufhuerfhffbveiluv.solid.community/profile/card#me"]'
